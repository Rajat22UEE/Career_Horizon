...
import { addInternship } from "../lib/api";

export default function InternshipForm({ onCreated }) {
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-gray-300 p-4 rounded-xl shadow-sm">
      <h3 className="font-bold text-black mb-3">Add Internship</h3>
   ...
   <input className="border border-gray-300 rounded-lg px-3 py-2 text-black bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300" placeholder="Stipend"
     value={form.stipend} onChange={(e)=>setForm({...form, stipend:e.target.value})} />
        ...
        <select className="border rounded-lg px-3 py-2"
                value={form.internshipType} onChange={(e)=>setForm({...form, internshipType:e.target.value})}>
